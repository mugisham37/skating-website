...
export default async function Page(props: {
  ...
  const defaultWheel =
    wheels.find((wheel: BoardCustomizerDocumentDataWheelsItem) => wheel.uid === searchParams.wheel) ?? wheels[0];
  const defaultDeck =
    decks.find((deck: BoardCustomizerDocumentDataDecksItem) => deck.uid === searchParams.deck) ?? decks[0];
  const defaultTruck =
    metals.find((metal: BoardCustomizerDocumentDataMetalsItem) => metal.uid === searchParams.truck) ?? metals[0];
  const defaultBolt =
    metals.find((metal: BoardCustomizerDocumentDataMetalsItem) => metal.uid === searchParams.bolt) ?? metals[0];

  const wheelTextureURLs = wheels
    .map((texture: BoardCustomizerDocumentDataWheelsItem) => asImageSrc(texture.texture))
    .filter((url: unknown): url is string => Boolean(url));
  const deckTextureURLs = decks
    .map((texture: BoardCustomizerDocumentDataDecksItem) => asImageSrc(texture.texture))
    .filter((url: unknown): url is string => Boolean(url));
