...
import { SettingsDocumentDataFooterSkateboardsItem, SettingsDocumentDataNavigationItem } from "../../prismicio-types";
...
export async function Footer() {
  ...
  const boardTextureURLs = settings.data.footer_skateboards
    .map((item: SettingsDocumentDataFooterSkateboardsItem) => asImageSrc(item.skateboard, { h: 600 }))
    ...
    <footer className="bg-texture bg-zinc-900 text-white overflow-hidden">
      ...
      <Bounded as="nav">
        <ul className="flex flex-wrap justify-center gap-8 ~text-lg/xl">
          {settings.data.navigation.map((item: SettingsDocumentDataNavigationItem) => (
